...
class SampleLogger(Callback):
    ...
    def log_sample(self, trainer, pl_module, batch):
        ...
        latent, info = model.encode(x, with_info=True)
        log_wandb_embeddings(logger=wandb_logger, id="z", embeddings=latent)

        if "bottleneck_bits" in info:
            bits = info["bottleneck_bits"]
            log_wandb_embeddings(logger=wandb_logger, id="bits", embeddings=bits)
