...
class Model(pl.LightningModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Predict autoregressively next embedding and return loss
...
class SampleLogger(Callback):
    ...
    def log_sample(self, trainer, pl_module, batch):
        ...
        latent = pl_module.autoencoder.encode(x_true)
        start_embedding = rearrange(latent, "b c t -> b t c")

        length = start_embedding.shape[-2]
        embedding_generated = pl_module.transformer.generate(
            start_embedding,
